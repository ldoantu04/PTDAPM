...
const sampleData = {
  navigationLinks: [
    ...
    { title: "Đội ngũ giảng dạy", link: "/doi-ngu-giang-day", active: true },
  ],
};
                      ...
                      <div className="p-[20px] md:p-[30px]">
                        <div className='flex gap-[20px] md:gap-[30px] flex-col md:flex-row'>
                          ...
                          <div className='w-full md:w-3/4'>
                            <div className="mb-4">
                              <p className="font-bold text-[22px] text-black mb-1">{staff.name}</p>
                              <p className="font-normal text-[16px] text-[#A3A3A3]">{staff.degree}</p>
                            </div>

                            <p className="font-normal text-[16px] mt-[15px]">{staff.department}</p>
              ...
              <div className='md:w-1/3'>
                <div className="border-2 border-gray-300 rounded-lg p-[30px]">
                  <div className="flex flex-col items-start mb-[30px]">
                    <p className="font-bold text-[22px] text-[#192E58] mb-[20px]">DANH SÁCH CHUYÊN GIA</p>
                    <div className="h-0.5 w-24 bg-red-600"></div>
                  </div>

                  {staffs.slice(0, 5).map((staff, index) => (
                    <div key={`expert-${staff._id}-${index}`} className="mb-6 flex items-start gap-2">
                      <div className="w-1/4">
                        {staff.thumbnail ? (
                          <img 
                            src={staff.thumbnail} 
                      ...
                      <div className="w-3/4">
                        ...
                        <p className="font-normal text-[14px] text-[#A3A3A3] mt-1">{staff.position}</p>
                        <p className="font-normal text-[14px] text-black mt-1">{staff.department}</p>
                      </div>
                    </div>
                  ))}
                </div>
